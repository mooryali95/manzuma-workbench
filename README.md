### v5.1.0 — تدقيق UI: نظام Design Tokens ✅
قياس قبل/بعد عبر كل ملفات CSS: أنصاف الأقطار من **14 قيمة → 5 رموز**، المقياس الطباعي من **14 حجماً (أدناها 9px) → 7 درجات (أدناها 10px)**، الألوان المؤسسية من **28 موضعاً يدوياً → صفر** (كلها من `tokens.css`)، ومقياس ظلال ثلاثي. إضافة: نعومة حروف، تحديد نص ذهبي، وأشرطة تمرير رفيعة موحدة. أي تعديل هوية مستقبلي يتم من ملف واحد.

### v5.0.0 — تدقيق UX وفق Nielsen/WCAG ✅
- **هوية موقعك واضحة**: روابط التنقل (الشجرة/المحافظ/الورشة) في مجموعة موحدة مع تمييز الصفحة الحالية بلون داكن + `aria-current`.
- **فصل الأفعال عن التنقل**: تصدير/استيراد/Baseline/المستخدمون انتقلت لقائمة «⚙ أدوات» منسدلة (تُغلق بالنقر خارجها أو Esc) — الترويسة انخفضت من 9 أزرار متنافسة لبنية من ثلاث فئات.
- **حالات فارغة قائدة**: «لا مفاهيم» في الشجرة أصبحت تحمل زر «+ أضف مفهوماً» ينقل للمحفظة.
- **WCAG**: تسميات aria للأزرار الأيقونية، حلقة تركيز ذهبية موحدة `:focus-visible`، وأهداف لمس 44px على الأجهزة اللمسية.
- **جوال**: روابط التنقل تتحول لأيقونات والبريد يختفي من الشريحة على الشاشات الضيقة.

### v4.9.0 — غرفة القيادة 🎦 ✅
ارتقاء فاخر للشجرة: حلقة تقدم دائرية ذهبية متحركة حول الجذر، أرقام تعدّ تصاعدياً، خطوط تفرع ترسم نفسها بتتابع، رؤوس محافظ متدرجة بعمق طبقي، وبطاقات ترتفع عند التمرير — كله يحترم prefers-reduced-motion. والإضافة: زر «🎦 وضع العرض» يقلب الشجرة لمسرح داكن (أخضر مجلسي + ذهبي متوهج + بطاقات زجاجية) بملء الشاشة لاجتماعات المجلس، والافتراضي يبقى الثيم الفاتح دائماً.
