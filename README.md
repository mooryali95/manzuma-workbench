### v5.7.0 — تقسيم workbench.js (مبدأ المسؤولية الواحدة) ✅
الملف (641 سطراً) قُسّم حسب المسؤولية لا الحجم: `workbench.js` (المنسّق + منطقة المفاهيم والتشكيلات، ~305 سطر) و`workbench-pool.js` (العمود الجانبي + نوافذ الأفراد/الكيانات، ~350 سطر). نقل آمن بلا تغيير منطق — تحقّق بفحص ESM شامل: كل رمز مستورد مُصدَّر فعلاً، صفر استيراد مكسور. الاستيراد الدائري بين الملفين آمن في ES Modules (استدعاء وقت التشغيل). تعديل العمود الجانبي صار لا يخاطر بمنطقة الورشة.

### v5.6.1 — تدقيق شامل وسدّ ثغرات ✅
فحص فعلي: 22 ملف JS سليم نحوياً، كل الاستيرادات النسبية صحيحة، 25 دالة adapter مستدعاة كلها معرّفة، 32 دالة store مستدعاة كلها موجودة، صفر ازدواج حقيقي. **ثغرتان مكتشفتان وأُصلحتا**: (1) عمليات التشكيلات (`actAddMember` وأخواتها) لم تكن تستدعي `assertWrite` — أي مقترِح/مشاهد قد يحاول تعديلها (الحماية النهائية في RLS لكن الواجهة لم تمنع) — أُضيف الحارس (6→10 حُرّاس). (2) رسالة مضلّلة للمقترِح عند الحذف — صارت دقيقة. وسُحب EXECUTE من anon عن دوال الصلاحية الجديدة.

### v5.6.0 — نظام المقترحات (Conservative Approval) ✅
دور جديد **«مقترِح» (contributor)**: يقرأ كل شيء لكنه لا يكتب مباشرة — كل تعديلاته تُحتجز في جدول `wb_proposals` كـ«معلّقة» دون لمس البيانات الأصلية، حتى يعتمدها المالك. (نموذج GitHub: write مقابل propose — المحررون المباشرون يبقون كما هم).
- **بوابة مركزية** في Store: أي `actCreate/actUpdate` من مقترِح يتحول تلقائياً لمقترح مع رسالة «أُرسل تعديلك كمقترح».
- **لوحة مراجعة للمالك** عبر «📥 المقترحات»: تعرض كل مقترح بـ«قبل ← بعد»، مع اعتماد (يطبّق فعلياً عبر RPC `SECURITY DEFINER` ويسجّل في التدقيق) أو رفض بسبب. شارة عدد حمراء + تحديث دوري.
- **أمان**: RLS — المقترِح يرى مقترحاته فقط، المالك يرى الكل؛ الاعتماد/الرفض للمالك حصراً. اختُبر منطق الاعتماد من الإنشاء حتى التطبيق على الجدول الفعلي.
- النطاق: إنشاء وتعديل العناصر والمفاهيم. (الحذف كمقترح والتشكيلات: تكرار لاحق).

### v5.5.0 — سجل التغييرات في الأدوات + إصلاح القوائم ✅
- **سجل التغييرات** خرج من صفحة المحافظ وصار عنصراً في قائمة «⚙ أدوات» يفتح كنافذة منبثقة — **للأدمن فقط**.
- **إصلاح القوائم المنسدلة**: كانت تبقى ظاهرة بسبب تسريب مستمعي أحداث عند كل إعادة رسم للترويسة. الحل: مستمع `document` واحد مفوّض (عبر عَلَم عام)، إغلاق تلقائي عند التنقل (hashchange)، ورفع z-index ليتموضع صحيحاً أسفل النوافذ.
- تحقق فعلي: نظام الدخول جاهز للأعضاء (أول مستخدم=مالك، تأكيد البريد معطّل، الأدوار والصفحات المخصصة تعمل).

### v5.4.0 — مفتاح الرموز + إصلاح وضع العرض ✅
- **خروج وضع العرض**: زر عائم ثابت «✕ خروج (Esc)» أعلى الشاشة + مفتاح Esc يعمل دائماً — حل غياب مخرج في ملء الشاشة.
- **أدوات نظيفة في العرض**: تصدير/استيراد/فتح/إغلاق/الأطوار تختفي تلقائياً أثناء العرض، يبقى البحث والتكبير فقط.
- **مفتاح الرموز (Legend)** أعلى الشجرة يشرح كل نوع (🏛 مجلس · 🗂 محفظة · 💡 مفهوم · 📦 منتج · 🚀 مبادرة · 🏗 مشروع) — ترميز مزدوج (أيقونة+لون) وفق أفضل ممارسات Data Viz، يتجاوز عمى الألوان ويعمل على اللمس. مع تلميح Hover على كل عنصر (النوع·الحالة·التقدم·المتأخرات).

### v5.3.0 — تدقيق فني وتحصين أمني ✅
تدقيق بأدوات Supabase Advisors الرسمية. **حرج**: قُفل جدولان كانا مكشوفين بلا RLS — `project_files` (نموذج صلاحيات الورشة) و`bot_settings` (مقفل عن API). أُزيلت سياسة مزدوجة على `wb_user_roles` ودالتان قديمتان من جلسات سابقة. **متوسط**: تحسين أداء سياسة RLS عبر `(select auth.uid())`، فهرس FK مفقود، تثبيت search_path. سُحب EXECUTE من anon عن دوال الصلاحية. النتيجة المؤكدة: صفر ثغرات XSS، صفر أسرار مكشوفة، وكل جداول البيانات الحساسة محجوبة عن anon (تحقق فعلي: صفر صفوف). توصية يدوية متبقية: تفعيل «حماية كلمات المرور المخترقة» من Dashboard ← Auth.

### v5.2.0 — التدقيق التكاملي UX×UI ✅
أربع فجوات في نقاط التقاء الشكل بالتفاعل: (1) **التحقق المرئي** — الحقل الإلزامي الفارغ يتوهج أحمر برسالة واهتزاز لطيف بدل إغلاق صامت كان **يُفقد المُدخلات** (خلل مكتشف: التحقق غير المتزامن كان يعيد Promise فيُغلق دائماً). (2) **منع الإرسال المزدوج** — زر التأكيد ينتظر الوعد فعلياً ويعرض «جارٍ الحفظ…» معطّلاً (سلامة بيانات). (3) **هيكل انتظار** متلألئ عند الإقلاع. (4) **كشف الخطر تلقائياً** — أزرار تأكيد الحذف تتلون حمراء آلياً + إصلاح تسرب لون الخطر للنوافذ التالية. مع انتقال صفحات ناعم يحترم تقليل الحركة.
